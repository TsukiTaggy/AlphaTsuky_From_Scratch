...
deterministic, synchronous PUCT search. This document describes only those
implemented boundaries.

## Current package boundaries

```text
YAML ----> azgo.config <---- azgo.cli ----> azgo.game
                 |              |               ^
                 |              v               |
                 |         azgo.search ----------+
                 |              |
            ...
            azgo.network <---- azgo.evaluator
                 ^                   |
                 |                   v
            azgo.symmetry       azgo.encoding ----> azgo.game
```
...
The Phase 1-4 schema covers only implemented behavior: board size, komi,
...
replay, learner, checkpoint, arena, and distributed settings are intentionally
...
The current commands validate configuration, benchmark legal engine play, and
analyze a move with uniform-evaluator MCTS. The benchmark uses an explicit
random seed and configured workload so runs can be reproduced. `search-move`
can reconstruct a state from repeatable row-major actions and optionally enable
seeded root noise; it emits a machine-readable JSON search report. Checkpoint
loading remains deferred to a later milestone.
...
- Every stochastic operation accepts an explicit seed. Zobrist tables,
  benchmark action selection, and optional root noise are therefore
  reproducible.
...
Concurrent inference queues, self-play, replay storage, learning, checkpoints,
arenas, SGF export, observability, and distributed workers are outside the
Phase 1-4 milestone. There are no placeholder implementations or configuration
sections for them.
